...
class DeblurDataset(Dataset):
    def __init__(self, dataset_folder, transform=None):
        ...
        self.image_files = [f for f in os.listdir(os.path.join(dataset_folder, 'blurred')) if f.endswith('.jpg')]
    ...
    def __getitem__(self, idx):
        ...
        blurred_image_path = os.path.join(self.dataset_folder, 'blurred', self.image_files[idx])
        ...
        blurred_image = torch.tensor([blurred_image]).permute(0, 3, 1, 2).float()
        sharp_image = torch.tensor([sharp_image]).permute(0, 3, 1, 2).float()
